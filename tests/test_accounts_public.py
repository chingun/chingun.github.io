...
def test_accounts_create(client):
    ...
    # Add a user
    avatar_path = os.path.join(utils.TEST_DIR, "testdata/fox.jpg")
...
def test_accounts_edit(client):
    ...
    assert users == [(
        ...
        "New Name",
        "newemail@umich.edu",
        "d6e98bee6a3ce58f012e71db8530dabdd628d5ddf252552f37662743616f81e5.jpg",
    )]
...
def test_accounts_password(client):
    ...
    response = client.post(
        "/accounts/login/",
        data={"username": "awdeorio", "password": "newpassword"},
    )
    assert response.status_code == 302
...
def test_accounts_delete(client):
    ...
    response = client.post(
        "/accounts/delete/",
        data={"delete": "confirm delete account"}
    )
    assert response.status_code == 302
    ...
    response = client.post(
        "/accounts/login/",
        data={"username": "jag", "password": "password"},
    )
